...
import type { OrderCreationRequest } from "@/features/orders/types";
import { useMutation } from "@tanstack/react-query";
import { ProductImagesApi } from "@/features/products/api";
import { useEffect } from "react";
import { CalendarDays, Clock, Car, Phone, Mail, MapPin, CreditCard, Truck, Loader2, Package } from "lucide-react";
  ...
  const createOrderMutation = useMutation({
    ...
    onSuccess: (response) => {
      toast({
        ...
        description: `Đơn hàng #${response.result?.id} đã được tạo. Chúng tôi sẽ liên hệ với bạn sớm.`,
      });
      form.reset();
      clearCart();
      onComplete();
